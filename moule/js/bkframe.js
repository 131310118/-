...
function loged() {
    if (window.profile) {
        $("#userName").each(function () {
            var o = this;
            if (!o.parentNode.menu) {
                o.parentNode.menu = createUserDropMenu({ "个人中心": "/hack", divider: "", "信息设置": "/hack/profile", "退出": "/logout" });
                o.parentNode.offsetY = 13;
                if (o.parentNode.menu)
                    Menu(o.parentNode);
            }
        })
...
function passwordView() {
    $("#passwordSection .edit-box", 0, 0).each(function () {
		var o = this;
        var active = 0;
        var eyeClass = ["proclaim-btn eye-slash", "proclaim-btn eye"];
        $("i", o).click(function () {
            o.input[active].className += " none";
            o.input[(active + 1) % 2].value = o.input[active].value;
            active = (active + 1) % 2;
            o.input[active].className = "wbox";

            o.input[active].focus();
            this.className = eyeClass[active];
        });
        o.input = $("input", o).focus(function () {
            var input = this;
            tipsing("<i></i>密码长度为6-32个字符", o.input[active]);
            input.tips.className = "alert-tips";
            if (!input.tips) {
                o.input[(active + 1) % 2].tips = o.input[active].tips
            }
        }).blur(function () {
            var input = this;
            o.input[(active + 1) % 2].value = o.input[active].value;
            if ($.valid("password", input.value)) {
                input.checked = true;
                input.tips.innerHTML = "";
            }
            else {
                input.checked = false;
                tipsing("<i></i>密码长度为6-32个字符", o.input[active]);
            }
            o.input[(active + 1) % 2].checked = input.checked;
        });
        tipsing("", o.input[0]);
        o.input[0].tips.style.display = "none";
        o.input[1].tips = o.input[0].tips;
    });
}
var format = {
    size: function (size) {
...
// 倒计时修改
function timerString(st){
    var nt = Date.parse(new Date())/1000;
    var sub=st-nt;
    var str="剩";
    var np=0;
...
function createUserPhoneMenu() {
    var globalnav = $("#globalnav")[0];
    if (!globalnav) return false;
    var phoneOp = $("#phoneOp");
    if (window.profile && window.profile.type !== undefined) {
        phoneOp[0].className = "navSubmit";
        phoneOp.append($.tag("a", {
            className: "btn btn-primary",
            innerHTML: "提交漏洞",
            href: global.host.root + "/bug/create"
        }));
        phoneOp.append($.tag("a", {
            className: "btn btn-default",
            innerHTML: "退出",
            href: global.host.root + "/logout"
        }));
    }
    else {
        phoneOp.append($.tag("a", {
            className: "btn btn-primary",
            innerHTML: "登录",
            href: global.host.root + "/signin"
        }));
        phoneOp.append($.tag("a", {
            className: "btn btn-default",
            innerHTML: "注册",
            href: global.host.root + "/signup"
        }));
    }
}
function createUserDropMenu(data) {//生成顶部用户名处下拉global菜单
    if (window.profile.type !== undefined) {
        if (window.profile.type != 0) {
            var menu = $.tag("ul", "dropdown-menu b1g");
            for (var key in data) { //{key:url}
                if (key != "divider") {
                    if (data[key].indexOf("javascript") < 0) {
                        data[key] = global.host.root + data[key];
                    }
                    menu.append("li", { innerHTML: "<a href=\"" + data[key] + "\">" + key + "</a>" });
                }
                else {
                    menu.append("li", "divider"); //分割线
                }
            }
        }
        else {
            var menu = $.tag("div", "dropdown-menu");
            var head = menu.append("div", "card-head");
            head.append("img", {
            ...
            head.append("p", {
                className: "meta",
                innerHTML: "<span>UID<b>" + format.uid(window.profile.uid) + "</b></span><a href=\"" + global.host.root + layout.href.mall + "\">虚拟币<b>" + window.profile.balance + "</b></a>"
            });

            var link = menu.append("div", "card-links");
            link.append("a", { innerHTML: "个人中心", href: global.host.root + layout.href.user.index });
            link.append("span", "split");
            link.append("a", {
                innerHTML: "我的名片", 
            ...
            if (window.profile.unread.msg) {
                var news = menu.append("div", "card-news").append("div", "new-edit mail");
                news.append("a", {
                    innerHTML: "<i class=\"fa fa-envelope\"></i>您有新的消息推送",
                    href: global.host.root + layout.href.user.message.list
                });
                news.append("span", { className: "count", innerHTML: window.profile.unread.msg });
            }
            if (window.profile.unread.apply) {
                var news = menu.append("div", "card-news").append("div", "new-edit mail");
                news.append("a", {
                    innerHTML: "<i class=\"fa fa-envelope\"></i>您有待处理的请求",
                    href: global.host.root + layout.href.user.apply.list
                });
                news.append("span", {
                    className: "count",
                    innerHTML: window.profile.unread.apply
                });
            }
            var footer = menu.append("div", "card-footer");
            footer.append("a", {
                innerHTML: "信息设置",
                href: global.host.root + layout.href.user.edit.info
            });
            footer.append("a", {
                innerHTML: "退出",
                // href: global.host.root + "logout"
...
function Header() {
    // if(window.layout.available.header) {
        ...
        if (window.profile && window.profile != null && window.profile.type !== undefined) {
            ...
            /*if (screenType.level >= 2)
                userName[0].href = global.host.root + window.src.menu.phone[profile.type]["home"].url;*/
...
function search(callback, options) {
    ...
    $("#searchBar").each(function (i, search) {
        ...
        function getTips(key) {//new
            if (key == '') return false;
            $.ajax({
                url: (options.url === '')? srcAjax.Home.Search.tips.url() : options.url,
                // data: {key: key, p: 1},
                data: {keyword: key},
                success: function(data) {
                    // data = srcAjax.Home.Search.tips.callback(data);
                    tips[0].innerHTML = "";
                    tips.data = [];
 ...
 * data:
 [
 {title: "aa", attr: {}, type: "input", name: "a", value: "2"},
...
var Tips = function () {
    ...
    this.show = function (config, callback) {
        ...
        if (options.confirm !== false) {
            ...
            if (!outer.confirm) {
                var confirm = outer.confirm = op.append("div", "btn-primary btn btn-sm mr10");
            ...
            if (outer.confirm) {
                outer.removeChild(outer.confirm);
                outer.confirm = null;
            }
        }
        if (options.cancel !== false) {
            if (!outer.cancel) {
                var cancel = outer.cancel = op.append("div", "btn btn-default btn-sm");
...
*分页功能
 */
function Pager(page, onPage, option) {
    config = {
        first: true,
        last: true,
        jump: true,
    ...
    //{id:,total:111,p:7,totalp:7}
    $.updateObject(option, config);
    page.id = page.id || "#pager";
    var base = $(page.id)[0];
    base.innerHTML = "";
    this.root = base;
    onPage = onPage || function () { };
    base.append = function (tag, className, innerHTML) {
        return base.appendChild($.tag(tag, {className: className, innerHTML: innerHTML}));
    };
    var main = base.append("ul", "cl");
    if (page.totalp <= 1) {
        main.style.display = "none";
    }
    var pageList;
    if (config.page)
        pageList = main.append("li");

    var jump;
    if (config.jump) {
        jump = main.append("li", "form_page");

        var value = jump.append("input", "page_input");
        value.type = "text";
        $(value).keydown(function (e) {
            e = e || window.event;
            if (e.keyCode == 13) {
                // page.p = value.value;
                page.p = (value.value >= page.totalp)? page.totalp : value.value;
                pageTo(page);
            }
        })
    }
    ...
    var total;
    if (config.total)
        total = main.append("li", "total").append("span", "ng-binding");


    var next;
    if (config.next) {
        next = main.append("li", "next");
        next.append("a", {innerHTML: "<span>下一页</span><i class=\"fa fa-chevron-right\"></i>"});
        $(next).click(function () {
            if (page.p < page.totalp) {
                page.p++;
                pageTo(page);
            }
        })
    ...
    var prev;
    if (config.prev) {
        prev = main.append("li", "pre");
        prev.append("a").append("i", "fa fa-chevron-left");
        $(prev).click(function () {
            if (page.p > 1) {
                page.p--;
                pageTo(page);
    ...
    var pageTo = this.pageTo = function (newpage, firstLoad) {
        var p = parseInt(newpage.p);
        if (p < 1) return false;
        if (!firstLoad) {
            onPage(p);
        }
    ...
    var update = this.update = function (newpage) {

        page = newpage;
        if(page.node){
            page.totalp = Math.ceil(page.total / page.node);
            page.p = page.current;
        }
        if (config.total)
            total.innerHTML = "/" + page.totalp + "页";
        if (page.totalp <= 1) {
            main.style.display = "none";
            return false;
        }
        ...
        if (config.page) {
            pageList.innerHTML = "";
            var start, end;
            var p = parseInt(page.p);
            if (page.totalp <= 5 || p <= 3) {
                start = 1;
                end = Math.min(page.totalp, 5) + 1;
            }
            else if (page.totalp - p < 3) {
                start = page.totalp - 4;
                end = page.totalp + 1;
            }
            else {
            ...
            if (config.first && start != 1) {
                var first = pageList.append("a", { innerHTML: "1.." });
                first.p = 1
            }

            for (var i = start; i < end; i++) {
                var newPage = pageList.append("a", { innerHTML: i });
                newPage.p = i;

                if (i == p) {
                    newPage.className += " on";
                }
            }

            if (config.last && end - 1 != page.totalp) {
                var last = pageList.append("a", { innerHTML: ".." + page.totalp });
                last.p = page.totalp
            }
